...
function buildHtml({
  ...
  // Line endings are normalised once, here, and every scanner downstream may
  // therefore assume "\n".
  //
  // git checks a document out with CRLF on Windows by default, and the
  // line-oriented passes below anchor with `$`, which in JavaScript matches
  // the end of the STRING -- while `.` refuses to match "\r". So a line ending
  // in "\r" silently fails to match a pattern that works everywhere else, and
  // the failure is invisible on the machine anyone develops on. Measured
  // before this was added: the footnote definition `[^why]: note` stopped
  // being recognised, so marked ate it as a link reference definition and the
  // note vanished from the document, and the fence scanner in inspect() saw no
  // fences at all -- which both zeroed the declared diagram count and let the
  // contents of quoted example fences be read as claims about the document.
  //
  // CRLF carries no meaning in Markdown and marked normalises it anyway, so
  // this changes nothing about the output beyond making it identical on every
  // platform.
