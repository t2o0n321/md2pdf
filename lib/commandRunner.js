...
class CommandRunner {
  ...
  which(command) {
    ...
    // The leading "" on Windows matters: without it a command that already
    // carries its extension ("chrome.exe", "npm.cmd") is only ever probed as
    // "chrome.exe.EXE" and is never found.
    const exts =
      process.platform === "win32"
        ? ["", ...(process.env.PATHEXT || ".COM;.EXE;.CMD;.BAT").split(";").filter(Boolean)]
        : [""];
