...
function extractDefinitions(source, defs) {
  // Split on either ending rather than relying on the caller. buildHtml
  // normalises before it gets here, but this is an exported entry point, and
  // the failure mode if a CRLF document reaches it is silent: DEFINITION ends
  // in `(.*)$`, `.` does not match "\r" and `$` wants the end of the string,
  // so every definition stops being recognised and every note is swallowed by
  // marked as a link reference definition.
