  ...
  const out = splitCode(md)
    .map((chunk, i) => {
      ...
      return chunk.replace(/!\[\[([^\]|]+?)(?:\|([^\]]*))?\]\]/g, (whole, rawTarget, suffix) => {
        ...
        const attrs = dims
          ? ` width="${dims[1]}"${dims[2] ? ` height="${dims[2]}"` : ""}`
          : "";
        // Always an <img>, never `![alt](url)`. Markdown syntax is not parsed
        // inside a raw HTML block, so an embed sitting in a hand-written
        // <table> -- the usual way two screenshots are put side by side --
        // printed as the literal text "![alt](file:///...)". HTML works in
        // both places and produces the same element either way.
        return `<img src="${url}"${attrs} alt="${escapeAttr(dims ? target : size || target)}">`;
      });
    })
    .join("");
